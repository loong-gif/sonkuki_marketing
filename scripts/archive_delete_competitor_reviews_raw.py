#!/usr/bin/env python3
"""Backup competitor_reviews_raw (m7k6bslqxmbw10a) to outputs/ then delete.
The 2007 reviews are identical to HDV1_Customer_Reviews competitor rows
(verified external_review_id set equality); product context lives in
HDV1_Review_Listing_Links. No other table links to this one.
"""
...
def main():
    ...
    backup = OUT_DIR / f"competitor_reviews_raw_archived_{stamp}.jsonl"
    ...
    print(f"tables now: {len(tables)}", flush=True)
    print(f"competitor_reviews_raw gone: {TABLE not in [t['id'] for t in tables]}", flush=True)
